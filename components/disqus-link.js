...
function installDisqusCount() {
  window.DISQUSWIDGETS = {
    getCount(id) {
      ...
      script.src = `https://${disqusShortname}.disqus.com/count-data.js?1=${encodeURIComponent(
        id,
      )}&${Date.now()}`;
...
export default class DisqusLink extends Component {
  ...
  render() {
    return (
      <a href={`${this.props.href}#disqus_thread`} data-disqus-identifier={this.props.id}>
        {this.state.text}
      </a>
    );
  }
  ...
  update = ({
    ...
    counts.forEach(({ comments: count, id }) => {
      ...
      const text = count === 0 ? zero : count === 1 ? one : multiple.replace('{num}', count);
